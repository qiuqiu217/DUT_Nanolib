...
//  }
	...
	if (_ucMode == 1 || _ucMode == 3)
	{
		...
		SaiOutHandle.Init.MonoStereoMode = SAI_STEREOMODE;          /* 立体声模式 */
		...
		SaiOutHandle.Init.ClockStrobing  = SAI_CLOCKSTROBING_RISINGEDGE;
...
*********************************************************************************************************
*/
uint32_t AUDIO_Play(int16_t* pBuffer1,int16_t* pBuffer2, uint32_t Size)
{		
	s_PlayFileBuf1 = pBuffer1;	/* 保存指针 */
    s_PlayFileBuf2 = pBuffer2;	/* 保存指针 */
	s_PlayFileLen = Size;		/* 保存文件长度 */
	
	/* Initialize the data buffer */
	for (int i = 0; i < PLAY_BUFF_SIZE / 2; i++)
	{
		s_PlayBuffCache[i] = s_PlayFileBuf1[i];
	}
	for (int i = 0; i < PLAY_BUFF_SIZE / 2; i++)
	{
		s_PlayBuffCache[i + PLAY_BUFF_SIZE / 2] = s_PlayFileBuf2[i];
	}
...
/*
*********************************************************************************************************
*	函 数 名: AUDIO_FillBuff
*	功能说明: 放音录音，。 用于讲文件缓冲区数据搬移到32字节对齐的DMA缓冲区.
*	形    参: pBuffer : 样本数据
*			  Size : 样本个数. 每通道16bit，双通道。每个样本占4字节。
*	返 回 值: 无
*********************************************************************************************************
*/
uint32_t AUDIO_FillBuff(FIL *_pFile, int16_t *_ptrtemp, uint16_t _usSize, uint8_t usBitsPerSample,uint8_t DmaState)
{
	...
	if(usBitsPerSample == 24)
	{
		...
		if(!DmaState)
		{
			...
			{
				s_PlayBuffCache[i]   = p[1];
				s_PlayBuffCache[i+1] = p[2]; 
			} 
		}
		else
		{
			for(i = 0; i < _usSize; i += 2, p += 3)
			{
				s_PlayBuffCache[i + PLAY_BUFF_SIZE / 2]   = p[1];
				s_PlayBuffCache[i + PLAY_BUFF_SIZE / 2+1] = p[2]; 
			} 
		}
	...
	else 
	{
		if(!DmaState)
		{
			f_read(_pFile, s_PlayBuffCache, _usSize, (UINT*)&NumBytesRead);
		}
		else
		{
			f_read(_pFile, s_PlayBuffCache + PLAY_BUFF_SIZE / 2, _usSize, (UINT*)&NumBytesRead);
		}	
	}
	...
	if(NumBytesRead < _usSize)
	{
		...
		else
		{
			...
			for(i = NumBytesRead; i < _usSize; i++)
			{
				s_PlayBuffCache[i + PLAY_BUFF_SIZE / 2] = 0; 				
			}
		}
	}
...
//				else
//				{
//					s_PlayBuffCache[i + PLAY_BUFF_SIZE / 2] = 0;		/* 不足补0静音 */
//					file_end = 1;
//				}
//			}
