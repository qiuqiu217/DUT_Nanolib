...
MP3DecInfo *AllocateBuffers(void)
{
  ...
  ClearBuffer((void*)&mp3DecInfo, sizeof(MP3DecInfo));
   ...
   // return mp3DecInfo_pointer;
	...
	mp3DecInfo_pointer = (MP3DecInfo *)rt_alloc_mem(AppMalloc, sizeof(MP3DecInfo));
	if (!mp3DecInfo_pointer)
		return 0;
	ClearBuffer(mp3DecInfo_pointer, sizeof(MP3DecInfo));
	
	fh =  (FrameHeader *)     rt_alloc_mem(AppMalloc, sizeof(FrameHeader));
	si =  (SideInfo *)        rt_alloc_mem(AppMalloc, sizeof(SideInfo));
	sfi = (ScaleFactorInfo *) rt_alloc_mem(AppMalloc, sizeof(ScaleFactorInfo));
	hi =  (HuffmanInfo *)     rt_alloc_mem(AppMalloc, sizeof(HuffmanInfo));
	di =  (DequantInfo *)     rt_alloc_mem(AppMalloc, sizeof(DequantInfo));
	mi =  (IMDCTInfo *)       rt_alloc_mem(AppMalloc, sizeof(IMDCTInfo));
	sbi = (SubbandInfo *)     rt_alloc_mem(AppMalloc, sizeof(SubbandInfo));
...
#define SAFE_FREE(x)	{if (x)	rt_free_mem(AppMalloc, x);	(x) = 0;}	/* helper macro */
#endif
