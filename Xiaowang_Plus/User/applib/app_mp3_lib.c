...
*                 解析和相关信息记录。
*              2. MP3MusicPlay的实现是基于uCOS-III操作系统的。
	...
	while(1)
	{
		...
		if(err == OS_ERR_NONE)
		{
			switch (xResult)
			{
				...
				case MusicTaskAudioPlay_5:
					...
					/* 初始化SAI音频接口进行播放 */
					AUDIO_Init(1, I2S_STANDARD_PHILIPS, SAI_DATASIZE_16, ptMP3->uiSamplerate, mp3FrameInfo.nChans);
		...
		if(res == FR_OK)
		{
			...
			{	
				...
				WM_SendMessageNoPara(GUI_TaskHwin, MSG_NextMusic);	
				return;			
			} 	
		}
	}
