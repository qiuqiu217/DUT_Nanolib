...
u8 flac_init(FIL* fx)
{
	...
	if(flacctrl->totsec)
	{
		flacctrl->outsamples=fc->max_blocksize*fc->channels;//PCM输出数据量(*2,表示2个声道的数据量)
		...
		if(err == OS_ERR_NONE)
		{
          ...
          if(fc->min_blocksize==fc->max_blocksize)//必须min_blocksize等于max_blocksize  &&fc->max_blocksize!=0
            ...
            f_read(&fout,flacctrl->FileBuff,fc->max_framesize,&br);//读取最大帧长数据	 
					...
					/* 初始化SAI音频接口进行播放 */
					AUDIO_Init(1, I2S_Standard_Phillips, SAI_DataSize_16b, flacctrl->samplerate, flacctrl->ch);
