#ifndef DATACOVERSIONUTILS_H
#define DATACOVERSIONUTILS_H

#include <QString>
class DataCoversionUtils
{
public:
    DataCoversionUtils();
    /**
     ...
     */
    static float u8Arry_float(uint8_t *data, bool key)
    {
        float fa = 0;
        uint8_t uc[4];
        if (key == true)
     ...
     * @param bool key  ture大端存储 false小端存储
     */
    static void float_u8Arry(uint8_t *u8Arry, float *floatdata, bool key)
    {
        uint8_t farray[4];
     ...
     */
    static void u32_u8Arry(uint32_t u32data, uint8_t *u8Arry)
    {
     ...
     */
    static uint32_t u8Arry_u32(uint8_t *u8Arry)
    {
        uint32_t u32data = 0;
        u32data |= (static_cast<uint32_t>(u8Arry[0]) << 24);
        u32data |= (static_cast<uint32_t>(u8Arry[1]) << 16);
        u32data |= (static_cast<uint32_t>(u8Arry[2]) << 8);
        u32data |= u8Arry[3];
        return u32data;
    }
     ...
     */
    static void u16_u8Arry(uint16_t u16data, uint8_t *u8Arry)
    {
     ...
     */
    static uint16_t u8Arry_u16(uint8_t *u8Arry)
    {
        uint16_t u16data = 0;
        u16data |= (static_cast<uint16_t>(u8Arry[0]) << 8);
        u16data |= u8Arry[1];
        return u16data;
    }
