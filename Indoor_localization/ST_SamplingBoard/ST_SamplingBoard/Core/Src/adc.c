...
void MX_ADC1_Init(void)
{
    ...
    */
    ...
    ADC_InitStruct.DataAlignment = LL_ADC_DATA_ALIGN_RIGHT;                   //右对齐方式
    ADC_InitStruct.SequencersScanMode = LL_ADC_SEQ_SCAN_DISABLE;              //失能扫描模式
    LL_ADC_Init(ADC1, &ADC_InitStruct);
    ADC_REG_InitStruct.TriggerSource = LL_ADC_REG_TRIG_SOFTWARE;              //软件自动触发
    ADC_REG_InitStruct.SequencerLength = LL_ADC_REG_SEQ_SCAN_DISABLE;         //失能ADC组常规序列器长度
    ADC_REG_InitStruct.SequencerDiscont = LL_ADC_REG_SEQ_DISCONT_DISABLE;     //失能ADC组顺序采集
    ADC_REG_InitStruct.ContinuousMode = LL_ADC_REG_CONV_CONTINUOUS;           //失能ADC连续转换模式
